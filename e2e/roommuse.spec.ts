...
// F12 — the priority chips filtered the list but left the headline figures describing the whole
// plan, so they could not inform the decision the filter exists to support.
test("the priority filter narrows the plan totals, not just the list",async({page})=>{
  await page.goto("/?demo=shopping");
  await expect(page.getByText("ESTIMATED PROJECT TOTAL")).toBeVisible();
  const money=page.getByText(/^\$[0-9,]+$/);
  const allTotal=await money.first().textContent();
  await page.getByRole("button",{name:"Essential",exact:true}).click();
  await expect(page.getByText("ESTIMATED TOTAL · ESSENTIAL")).toBeVisible();
  const essentialTotal=await money.first().textContent();
  expect(essentialTotal).not.toEqual(allTotal);
  // Returning to All restores the whole-plan figure.
  await page.getByRole("button",{name:"All",exact:true}).click();
  await expect(page.getByText("ESTIMATED PROJECT TOTAL")).toBeVisible();
  expect(await money.first().textContent()).toEqual(allTotal);
});
