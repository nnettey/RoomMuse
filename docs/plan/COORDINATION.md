...
| WS-4 | Persistence & project store | Lead session | `ws/4-persistence` | **Done** | WS-1 ✓ |
| WS-5 | UI | Lead session | `ws/5b-shopping-companion` | **Done** — 5a and 5b complete | WS-1 ✓ WS-3 ✓ WS-4 ✓ |
...
| `listProjects`, `listSummaries`, `summarise`, `openProject`, `saveToLibrary`, `renameProject`, `deleteProject`, `setProjectStatus`, `stripInlineImages`, `estimatePayloadBytes` | WS-4 | WS-5 | **Published** — `src/projectStore.ts` |
...
| 2026-08-17 | WS-5b | **Shopping companion done.** New `src/ShopScreens.tsx`: a constraints screen (add, view what each protects, deliberate confirmed release); a rebuilt `FieldScreen` that photographs a product, identifies it through the vision endpoint, lets the user name which planned item it replaces or add it as new, **previews the effect on the plan and budget before anything is committed**, and applies it; and comparison of products or variations with favourites. The old `FieldScreen` canned-string implementation is gone. Gates: typecheck clean, unit 70/70, **e2e 15/15**. |
| 2026-08-17 | WS-5a | **The spine is reachable.** New `src/PlanScreens.tsx`: numeric budget with live variance, category rollup and over-budget cost drivers; saved-project library with lifecycle; price refresh showing previous vs current with dates, delta and effect on the total. Shopping now routes through the budget step and `POST /api/shopping-plan`, so the plan is built for the chosen variation with the budget and constraints applied. Capture copy now matches the S-1 finding. **Two real bugs caught by the new journeys:** rebuilding a plan discarded the user's keep-this decisions (constraints are now re-applied, and the legacy refine toggles become durable constraints); and the budget never reached the server because `save` is debounced, so the updated project now travels with the call. Gates: typecheck clean, unit 70/70, **e2e 12/12**. |
| 2026-08-17 | WS-2 | **`/api/prices/refresh` and `/api/identify-product` shipped.** Refresh re-checks the SAME product page and accepts a figure only if the model cited that exact URL; completed projects are refused with 409 at the API as well as in the domain. Identification never invents a measurement, price, brand or retailer. **Live proof:** a completed project was refused; an item with no product page failed honestly with no invented price; a real refresh detected a genuine change (rug $118.99 → $116.99, "In stock — 6 left"); identification returned the sectional with dimensions **withheld** (`dimensionsConfidence: unknown`) and a price only when the shopper supplied one. WS-5 now has real APIs to build against. |
| 2026-08-17 | WS-4 | **Done.** New `src/projectStore.ts` (list/open/rename/delete/status, summaries), `roommuse.project.v3` with additive idempotent migration, and v1/v2 keys still read and never deleted. **Caught a defect that would have made WS-1 and WS-3 invisible:** `normalizeProject` built an explicit object and silently dropped all 11 v3 fields, so budget, constraints, favorites, status and room context were erased on every save/reload. Conflict resolution now unions price history and never loses a constraint release. **Risk R8 measured and closed:** a real 3-photo project is 825 KB inline, so 8 rooms breach Android's ~6 MB AsyncStorage ceiling; library entries now store photos by reference (33 KB, 25x smaller) while the active project keeps its inline copies. Gates: typecheck clean, **unit 70/70**, e2e 9/9. |
