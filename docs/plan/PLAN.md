...
| R8 | ~~Project payloads embed base64 images; limits untested~~ | **Closed 2026-08-17** | Measured: 825 KB per 3-photo project inline, so 8 rooms breach Android's ~6 MB AsyncStorage ceiling. Library entries now store photos by reference (33 KB) when the server already holds the scan; the active project keeps its inline copies, and a photo with no server copy is never stripped |
