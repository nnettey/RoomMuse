import {concepts as demoConcepts,styles} from "./enhancedData";import {Alternative,Concept,ConceptName,Filter,Item,Priority,Progress,Project,Report,RoomAnalysis,Sort,Style} from "./enhancedTypes";import {DesignConcept as LegacyConcept} from "./types";import{assetUrl}from"./assetUrl";
...
export function progress(ms:number,ready=false,error?:string):Progress{
  const stage=ready?7:Math.min(6,Math.floor(ms/8000));
  const completedStages=Array.from({length:stage},(_,i)=>i);
  if(error)return{currentStage:stage,completedStages,status:"error",error};
  return{
    currentStage:stage,
    completedStages,
    status:ready&&stage===7?"complete":ms>75000?"delayed":"working",
    message:ms>75000&&!ready?"Your room is taking a little longer to compose. We\u2019re still working.":undefined
  };
...
export const swapItem=(c:Concept,id:string,a:Alternative)=>updateItem(c,id,{name:a.name,retailer:a.retailer,purchaseUrl:a.purchaseUrl,unitPrice:a.unitPrice,dimensions:a.dimensions,finish:a.finish,rationale:"This alternative preserves the item's functional role and category while offering a "+a.difference.toLowerCase()+". Confirm retailer specifications before ordering."});
...
export function normalizeItem(i:Item,conceptId?:string):Item{return{...i,conceptId:i.conceptId??conceptId,dimensions:i.dimensions??"Confirm retailer dimensions",rationale:i.rationale??"Selected to support the concept's palette and functional plan. Confirm dimensions and availability before ordering.",matchIndicators:i.matchIndicators??[],priority:i.priority??"High impact",isOwned:i.isOwned??false,isPurchased:i.isPurchased??i.checked??false,isRemoved:i.isRemoved??false,alternatives:i.alternatives??[]};}
  ...
  if(legacy){
    ...
    for(const variant of server.variants??[]){
      ...
      if(index>=0){
        ...
        cs[index]={...current,imageDataUrl:assetUrl(variant.imageDataUrl??cs[0]?.imageDataUrl),designReport:variant.designReport??current.designReport,roomAnalysis:server.roomAnalysis,generationStatus:variant.generationStatus==="complete"?"complete":"partial"};
      }
    }
  }
...
export function applyBudget(p:Project,tier:import("./enhancedTypes").BudgetTier):Project{const c=selected(p);if(!c)return p;const factor=tier==="save"?.65:tier==="balanced"?.85:1;const items=c.shoppingItems.map(item=>{if(tier==="invest"||item.isOwned||item.isRemoved)return item;const cheaper=(item.alternatives??[]).filter(a=>a.unitPrice<=item.unitPrice*factor).sort((a,b)=>b.unitPrice-a.unitPrice)[0];return cheaper?{...item,name:cheaper.name,retailer:cheaper.retailer,purchaseUrl:cheaper.purchaseUrl,unitPrice:cheaper.unitPrice,dimensions:cheaper.dimensions,finish:cheaper.finish,rationale:"Budget-compatible alternative selected for the approved decor."}:item});return{...replace(p,{...c,shoppingItems:items}),budgetTier:tier}}
